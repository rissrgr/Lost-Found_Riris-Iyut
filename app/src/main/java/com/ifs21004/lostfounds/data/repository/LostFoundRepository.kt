package com.ifs21004.lostfounds.data.repository
...
//    Fungsi untuk menambahkan cover menggunakan pemanggilan cover dari API
    fun addCoverLostFound(
        ...
        try {
            //get success message
            emit(MyResult.Success(apiService.addCover(todoId, cover)))
        } catch (e: HttpException) {
            //get error message
            val jsonInString = e.response()?.errorBody()?.string()
            emit(
